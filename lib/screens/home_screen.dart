...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _init() async {
    ...
    _sub = BackgroundServiceManager.statsStream.listen((d) {
      if (!mounted) return;
      setState(() {
  ...
  void didChangeAppLifecycleState(AppLifecycleState s) {
    if (s == AppLifecycleState.resumed) {
      BackgroundServiceManager.isRunning().then((v) {
        if (mounted) setState(() => _svcRunning = v);
      });
    }
  }

  bool get _isConnected => _status == 'connected';
  bool get _isConnecting => _status == 'connecting';

  // ── colors by status ──────────────────────────────────────
  Color get _accentColor => switch (_status) {
        'connected' => const Color(0xFF00F5C4),
        'connecting' => const Color(0xFFFFB627),
  ...
  Future<void> _toggleService() async {
    if (_svcRunning) {
      // Send stop command to background isolate
      BackgroundServiceManager.stop();

      // Wait for service to actually stop (poll up to 3s)
      for (int i = 0; i < 15; i++) {
        await Future.delayed(const Duration(milliseconds: 200));
        final still = await BackgroundServiceManager.isRunning();
        if (!still) break;
      }
      ...
      // init() handles configure (once) + startService
      await BackgroundServiceManager.init();

      // Poll until service confirms it's up (up to 3s)
      for (int i = 0; i < 15; i++) {
        await Future.delayed(const Duration(milliseconds: 200));
      ...
      final running = await BackgroundServiceManager.isRunning();
      if (mounted) setState(() => _svcRunning = running);
    }
  }

  void _reconnect() => BackgroundServiceManager.reconnect();
          ...
          children: [
            ...
            AnimatedContainer(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(
                  _svcRunning ? 'SERVICE ON' : 'SERVICE OFF',
                  style: TextStyle(
  ...
  Widget _buildOrb() => SizedBox(
        ...
        child: Stack(alignment: Alignment.center, children: [
          // Outer rotating ring (only when connecting/connected)
          ...
          AnimatedBuilder(
            ...
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(
                  color: _accentColor.withOpacity(0.15 * _pulseAnim.value),
                  width: 1,
                ),
              ),
            ),
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border:
                  Border.all(color: _accentColor.withOpacity(.6), width: 1.5),
              boxShadow: [
                BoxShadow(
                  color: _accentColor.withOpacity(_isConnected ? .25 : .08),
              ...
              child: AnimatedSwitcher(
                ...
                child: _isConnecting
                    ...
                    : Icon(
                        ...
                        _isConnected ? Icons.hub_rounded : Icons.hub_outlined,
                        color: _accentColor,
                        size: 36,
                      ),
              ),
  ...
  String _statusTitle() => switch (_status) {
        'connected' => 'Connected',
  ...
  String _statusSubtitle() => switch (_status) {
  ...
  Widget _buildActionRow() => Padding(
        ...
        child: Row(children: [
          ...
          Expanded(
            flex: 3,
            child: _GlowButton(
              label: _svcRunning ? 'Stop Service' : 'Start Service',
              accent: _svcRunning
...
class _MetricCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: const Color(0xFF0D1117),
          border: Border.all(
            color: active ? accent.withOpacity(.25) : const Color(0xFF1C2333),
            width: 1,
          ),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Icon(icon,
              color: active ? accent : const Color(0xFF2D3748), size: 16),
          const SizedBox(height: 10),
          Text(value,
              style: TextStyle(
                color:
                    active ? const Color(0xFFEAEEF4) : const Color(0xFF2D3748),
                fontSize: 18,
                fontWeight: FontWeight.w600,
                fontFamily: 'monospace',
                letterSpacing: .5,
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: widget.accent.withOpacity(.5), width: 1),
...
class _IconActionButtonState extends State<_IconActionButton> {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          height: 52,
          decoration: BoxDecoration(
            color: _pressed ? const Color(0xFF1C2333) : const Color(0xFF0D1117),
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final dotX = size.width * (.2 + .6 * (math.sin(t * 2 * math.pi) * .5 + .5));
    final dotY =
        size.height * (.1 + .6 * (math.cos(t * 2 * math.pi * .7) * .5 + .5));
    final radPaint = Paint()
      ...
      ).createShader(Rect.fromCircle(center: Offset(dotX, dotY), radius: 120));
